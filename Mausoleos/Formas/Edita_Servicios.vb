...
#End Region
    Private Sub Edita_Servicios_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Carga_Combobox()
    End Sub
#Region "Carga Datos"
    Sub Carga_Combobox()
        Using Cx As New SqlConnection(CxSettings.ConnectionString)
            SQL_Str = "Select Servicio, Id_Servicio from Servicios Where Id_Servicio in" &
    ...
    Sub Carga_Disponibles()
        ...
        End Try
        Using Cx As New SqlConnection(CxSettings.ConnectionString)
            SQL_Str = "Select Utilizados as Total from Servicios_UtilizadosyDisponibles Where Id_Contrato = @id_Contrato and Id_Servicio = @Id_Servicio"
            Try
                Cx.Open()
                Dim Cmd As New SqlCommand(SQL_Str, Cx)
                Cmd.CommandType = CommandType.Text
